...
public:
    ...
    // fast_gicp::FastVGICP<pcl::PointXYZI, pcl::PointXYZI> vgicp_mt;

    fast_gicp::FastVGICP<pcl::PointXYZI, pcl::PointXYZI> vgicp_mt;
    void segmentationGround(const pcl::PointCloud<pcl::PointXYZI> &pcl_cloud)
    {
        double time_taken;
        ...
        PatchworkppGroundSeg->estimate_ground(pcl_cloud, pc_ground, pc_non_ground, time_taken);
    ...
    void laserCloudInfoHandler(const liorf::cloud_infoConstPtr &msgIn)
    {
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)
        {
            ...
            vgicp_mt.setMaximumIterations(50);
            vgicp_mt.setResolution(1.0);
            vgicp_mt.setNumThreads(8);

           
            // 用于判断点云是否是地面点还是非地面点
            segmentationGround(*laserCloudSurfLast);
            ...
            // cout << laserCloudSurfFromMapDS->empty() << endl;

            // vgicp_mt.align();
            // cout << laserCloudSurfLastDS->empty() << endl;
            // cout << target->points.empty() << endl;
            //    cout<<laserCloudSurfLastDS->points.empty()<<endl;
            //    cout<<laserCloudSurfFromMapDS->points.empty()<<endl;
            //    cout<<(*laserCloudSurfFromMapDS).size()<<endl;
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    bool LMOptimization(int iterCount)
    {
        ...
        // cout<<transformTobeMapped[3]<<" "<<transformTobeMapped[4]<<" "<<transformTobeMapped[5]<<endl;
    ...
    bool CloudMatching(
        Registration &reg, const pcl::PointCloud<pcl::PointXYZI>::Ptr &target,
        const pcl::PointCloud<pcl::PointXYZI>::Ptr &source)
    {
        if (laserCloudSurfLastDSNum > 30)
        {
            ...
            // Guess(1,1)=-1;
            // Guess(0,0)=-1;
            Guess.block<3, 3>(0, 0) = transGuess.matrix().block<3, 3>(0, 0);
            Guess(0, 3) = transformTobeMapped[3];
            Guess(1, 3) = transformTobeMapped[4];
            Guess(2, 3) = transformTobeMapped[5];
            ...
            ;
            trans.translation() << rough_trans_(0, 3), rough_trans_(1, 3), rough_trans_(2, 3);
            trans.rotate(rough_trans_.block<3, 3>(0, 0));
            pcl::getTranslationAndEulerAngles(trans, transformTobeMapped[3], transformTobeMapped[4], transformTobeMapped[5],
                                              transformTobeMapped[0], transformTobeMapped[1], transformTobeMapped[2]);
    ...
    void scan2MapOptimization()
    {
        ...
        if (laserCloudSurfLastDSNum > 30)
        {
            ...
            CloudMatching(vgicp_mt, laserCloudSurfFromMapDS, laserCloudSurfLastDS);
