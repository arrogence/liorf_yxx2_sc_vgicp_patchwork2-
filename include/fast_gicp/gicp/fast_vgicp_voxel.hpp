...
static std::vector<Eigen::Vector3i, Eigen::aligned_allocator<Eigen::Vector3i>> neighbor_offsets(NeighborSearchMethod search_method) {
  switch(search_method) {
    ...
    //   };
        case NeighborSearchMethod::DIRECT1:
    {
 std::vector<Eigen::Vector3i, Eigen::aligned_allocator<Eigen::Vector3i>> temp;
      temp.push_back(Eigen::Vector3i(0, 0, 0));
      return temp;
    }
        ...
        case NeighborSearchMethod::DIRECT7:
    {
           std::vector<Eigen::Vector3i, Eigen::aligned_allocator<Eigen::Vector3i>> temp;
      temp.push_back(Eigen::Vector3i(0, 0, 0));
      temp.push_back(Eigen::Vector3i(1, 0, 0));
      temp.push_back(Eigen::Vector3i(-1, 0, 0));
      temp.push_back(Eigen::Vector3i(0, 1, 0));
      temp.push_back(Eigen::Vector3i(0, -1, 0));
      temp.push_back(Eigen::Vector3i(0, 0, 1));
      temp.push_back(Eigen::Vector3i(0, 0, -1));
      return temp;
    }
...
public:
  ...
  // }
 Eigen::Vector3i voxel_coord(const Eigen::Vector4d& x) const {
    Eigen::Vector4d x2=x.array();
    for(int i=0;i<x2.size();i++)
    {
      if(x2[i]>=0)
      {
        x2[i]=(int)(x2[i]/ voxel_resolution_ - 0.5);
      }
      else
      {
        x2[i]=(int)(x2[i]/ voxel_resolution_ - 0.5)-1;
      }
    }
    ...
    voxel[2]=x2[2];
    return voxel;
    // return x2.template cast<int>()[0,3];
  }
